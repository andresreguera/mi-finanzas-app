...
# --- 3. CONEXIÓN ORDENADA (SEGÚN TU EXCEL) ---
@st.cache_resource
def conectar():
    try:
...
# --- LÓGICA DE CONEXIÓN POR ORDEN ---
# 1. Primera Pestaña: MOVIMIENTOS
hoja_movimientos = libro.get_worksheet(0) # Siempre coge la primera hoja

# 2. Segunda Pestaña: DEUDAS
try:
    hoja_deudas = libro.worksheet("Deudas")
except:
    # Si no existe, la crea
    hoja_deudas = libro.add_worksheet(title="Deudas", rows="100", cols="5")
    hoja_deudas.append_row(["Fecha", "Persona", "Concepto", "Monto", "Tipo"])

# 3. Tercera Pestaña: OBJETIVOS
try:
    hoja_obj = libro.worksheet("Objetivos")
except:
    # Si no existe, la crea
    hoja_obj = libro.add_worksheet(title="Objetivos", rows="100", cols="5")
    hoja_obj.append_row(["Objetivo", "Monto_Meta", "Fecha_Limite", "Fecha_Creacion"])
...
# --- 5. BARRA LATERAL (REPARACIÓN) ---
with st.sidebar:
    st.header("Configuración")
    if st.button("⚠️ REPARAR TABLAS (REINICIO TOTAL)", type="primary"):
        # Limpiamos y reescribimos encabezados en ORDEN
        # 1. Movimientos
        hoja_movimientos.clear()
        hoja_movimientos.append_row(["Fecha", "Categoría", "Concepto", "Monto", "Tipo"])
        ...
        st.cache_data.clear()
        st.success("Tablas reparadas y ordenadas.")
        st.rerun()
...
try:
    data = hoja_movimientos.get_all_records(numericise_ignore=['all'])
...
# --- 8. PESTAÑAS (ORDENADAS COMO EN TU EXCEL) ---
tab1, tab2, tab3, tab4 = st.tabs(["📝 Diario", "💸 Deudas", "🎯 Metas", "📊 Reporte"])

# === PESTAÑA 1: DIARIO (MOVIMIENTOS) ===
with tab1:
        ...
        with st.form("mov", border=False):
            ...
            if st.form_submit_button("Guardar Movimiento", use_container_width=True):
                if val > 0:
                    ...
                    hoja_movimientos.append_row([fecha.strftime("%d/%m/%Y"), cat, desc, val_excel, tipo])
                    st.toast("Guardado")
                ...
                with st.container():
                    ...
                    with col_num:
                        if falta > 0 and dias > 0:
                            ...
                            st.write(f"{al} **{formato_visual(men)}/mes**")
